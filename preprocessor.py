...
# M/D/YY, H:MM[:SS] [am|pm] - Name: message  (12h or 24h; seconds default :00)
...
def _dash_groups_to_row(month, day, y_raw, hour, minute, sec, meridiem, name, message):
    month, day = int(month), int(day)
    hour, minute = int(hour), int(minute)
    hour = _hour_12_to_24(hour, meridiem)
...
def _preprocess_dash(data):
    ...
    for raw_line in data.splitlines():
        ...
        if match:
            flush()
            m, d, y, hh, mm, ss, meridiem, name, message = match.groups()
            message = message.replace("\u200e", "").strip()
            current = [m, d, y, hh, mm, ss, meridiem, name, [message] if message else []]
        elif current is not None:
